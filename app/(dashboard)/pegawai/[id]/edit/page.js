...
export default function EditPegawai() {
  ...
  try {
    const [pegawaiRes, jabatanRes, departemenRes] = await Promise.all([
      api.get(`/pegawai/${id}`),
      api.get('/master-data?tipe=jabatan'),
      api.get('/master-data?tipe=departemen'),
    ]);

    const data = pegawaiRes.data.data;
    
    // 🔥 KONVERSI status_kawin dari database ke frontend
    let statusKawinValue = data.status_kawin;
    if (data.status_kawin === 'kawin') statusKawinValue = 'Menikah';
    if (data.status_kawin === 'tidak kawin') statusKawinValue = 'Belum Menikah';
    ...
    setForm({
      ...
      jarak_rumah_kantor: data.jarak_rumah_kantor || '',
      tanggal_lahir: data.tanggal_lahir ? data.tanggal_lahir.split('T')[0] : '',
      status_kawin: statusKawinValue, // <-- PAKAI YANG SUDAH DIKONVERSI
      ...
      status_kontrak: data.status_kontrak || 'kontrak',
              ...
              <div className="flex items-center gap-4">
                {(fotoPreview || existingFoto) && (
                  <div className="relative w-20 h-20 rounded-full overflow-hidden border-2" style={{ borderColor: 'var(--border-color)' }}>
                    <img 
                      src={fotoPreview || `/uploads/${existingFoto}`} 
                      alt="Preview" 
                ...
                <label className="px-4 py-2 rounded-lg text-sm cursor-pointer" style={{
                  ...
                  <input
                    ...
                    className="hidden"
                  />
                  {existingFoto ? 'Ganti Foto' : 'Pilih Foto'}
                </label>
              </div>
